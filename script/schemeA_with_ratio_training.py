...
Example: python schemeA_with_ratio_training.py gene_presence_absence.Rtab,snp_sites.vcf metadata_checkm.csv -t pres+vcf -s train_test_split_amoxicillin.tsv -A amoxicillin -o lightgbm -P params_saved 
                  """)
...
# open(output_file_name + '/log.txt','a')=open(output_file_name + '/log.txt','a')
...
for a in antis:
    ...
    for clade in included_clade:
        ...
        # when considering ratios, there are two major ones: 0: R1+S1+ ratio*Rn+ (1-ratio)*Sn; 2: Rn+Sn+ratio*R1+(1-ratio)*S1
        for case in range(2):
            ...
            for f in range(folds):
                ...
                for r in ratio:
                    train_test_split_cols.append('train_test_split_'+str(length+folds*case*len(ratio)+f*len(ratio)+ratio.index(r))) #f+length+folds*ratio.index(r)
                    clade_split_ratio_fold['train_test_split'].append(length+folds*case*len(ratio)+f*len(ratio)+ratio.index(r))
                    ...
                    clade_split_ratio_fold['ratio'].append(r)
                    ...
                    if case==0:#0: R1+S1+ ratio*Rn+ (1-ratio)*Sn; 1: Rn+Sn+ratio*R1+(1-ratio)*S1
                        Rn_r=list(random.sample(Rns[f][0],k=int(len(Rns[f][0])*r)))
                        Sn_r=list(random.sample(Sns[f][0],k=int(len(Sns[f][0])*(1-r))))
                        training_inds=R1s[f][0]+S1s[f][0]+Rn_r+Sn_r
                        extra_test_inds=R1s[f][1]+S1s[f][1]+[i for i in list(R_samples_cn.index) if i not in Rn_r]+[i for i in list(S_samples_cn.index) if i not in Sn_r]
                        clade_split_ratio_fold['R1'].append(len(R1s[f][0]))
                        clade_split_ratio_fold['S1'].append(len(S1s[f][0]))
                        clade_split_ratio_fold['Rn'].append(len(Rn_r))
                        clade_split_ratio_fold['Sn'].append(len(Sn_r))
                    if case==1:#1: Rn+Sn+ratio*R1+(1-ratio)*S1
                        R1_r=list(random.sample(R1s[f][0],k=int(len(R1s[f][0])*r)))
                        S1_r=list(random.sample(S1s[f][0],k=int(len(S1s[f][0])*(1-r))))
                        training_inds=R1_r+S1_r+Rns[f][0]+Sns[f][0]
                        extra_test_inds=Rns[f][1]+Sns[f][1]+[i for i in list(R_samples_c1.index) if i not in R1_r]+[i for i in list(S_samples_c1.index) if i not in S1_r]
                        clade_split_ratio_fold['R1'].append(len(R1_r))
                        clade_split_ratio_fold['S1'].append(len(S1_r))
                        clade_split_ratio_fold['Rn'].append(len(Rns[f][0]))
                        clade_split_ratio_fold['Sn'].append(len(Sns[f][0]))
                    for i in training_inds:
                        training.at[i,'train_test_split_'+str(length+folds*case*len(ratio)+f*len(ratio)+ratio.index(r))]='training'
                    for i in extra_test_inds:
                        training.at[i,'train_test_split_'+str(length+folds*case*len(ratio)+f*len(ratio)+ratio.index(r))]='extra_test'
    ...
    for f in range(len(train_test_split_cols)):
        ...
        test_predictions['train_test_split']+=list(test['train_test_split'])
