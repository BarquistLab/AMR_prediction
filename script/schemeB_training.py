...
Example: python schemeB_training.py gene_presence_absence.Rtab,snp_sites.vcf metadata_checkm.csv -t pres+vcf -s train_test_split_amoxicillin.tsv -A amoxicillin -o lightgbm -P params_saved 
...
for a in antis:
    ...
    '''
    # The aim of the split is to design the train-test split to see the effect of population structure with more training samples
    # Steps:
    # 1. Save 20 R + 20 S from clade_n for testing
    # 2. With the same test set, test the performance when including all samples from all other clades and only R or S samples from clade_n
    '''
    for clade in included_clade:
        length=len(train_test_split_cols)
        X_training=training[training["genome_id"].isin(list(split_file[split_file['split']!=clade]['genome_id']))] ## use all other clades as training
        X_clade=training[training["genome_id"].isin(list(split_file[split_file['split']==clade]['genome_id']))] 
            
        # 2 cases for training: 0: R_allother+S_allother+Rn ; 1:R_allother+S_allother+Sn; 
        for case in range(2):
    ...
    if params_output!=None and os.path.isfile(params_output+"/"+a+"_params.pkl"):
        ...
        # params={'bagging_fraction': 0.6064672018774342, 'feature_fraction': 0.1966979885235144, 'feature_fraction_bynode': 0.7171942211254374, 'lambda_l2': 0.22992898723842004, 'learning_rate': 0.225883398743703, 'max_bin': 750, 'max_depth': 28, 'min_data_in_leaf': 44, 'num_leaves': 30, 'verbose': -1, 'objective': 'binary', 'boosting': 'gbdt', 'linear_tree': False, 'tree_learner': 'serial', 'num_threads': 0, 'seed': None, 'deterministic': True, 'force_col_wise': True, 'bagging_seed': None, 'feature_fraction_seed': None, 'extra_trees': True, 'num_iterations': 100}
    ...
    # estimator=HistGradientBoostingClassifier(random_state=np.random.seed(random_seed))
